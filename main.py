...
with main:
    ...
    metrics = [{'label': 'Likes', 'value': round(df['totalNumberOfLikes'].iloc[-1]), 'delta': round(df['totalNumberOfLikes' + daily_change_reference].iloc[-1]) if 'totalNumberOfLikes' + daily_change_reference in df.columns and not pd.isna(df['totalNumberOfLikes' + daily_change_reference].iloc[-1]) else 0},
               ...
               {'label': 'Users', 'value': round(df['totalNumberOfRegisteredUsers'].iloc[-1]), 'delta': round(df['totalNumberOfRegisteredUsers' + daily_change_reference].iloc[-1]) if 'totalNumberOfRegisteredUsers' + daily_change_reference in df.columns and not pd.isna(df['totalNumberOfRegisteredUsers' + daily_change_reference].iloc[-1]) else 0},
               {'label': 'Libraries', 'value': round(df['totalNumberOfLibraries'].iloc[-1]), 'delta': round(df['totalNumberOfLibraries' + daily_change_reference].iloc[-1]) if 'totalNumberOfLibraries' + daily_change_reference in df.columns and not pd.isna(df['totalNumberOfLibraries' + daily_change_reference].iloc[-1]) else 0}]
